...
Plans:
- [ ] 04-01-PLAN.md — Postprocessing pipeline: install @react-three/postprocessing@~3.0.4 + ScenePostprocessing wrapper (PerformanceMonitor binary tier) + lazy PostFX (Bloom + Scanline + CA + Noise verified prop names) + size-limit PostFX entry + ThreeDShell mount (3D-08)
- [ ] 04-02-PLAN.md — Web3Forms ContactForm: src/lib/web3forms.ts POST helper + src/ui/ContactForm.tsx (idle/submitting/sent/failed + honeypot silent abort) + tests + CSP extension in index.html + .env.example + mount in src/sections/Contact.tsx (CTC-01)
- [ ] 04-03-PLAN.md — Live Profiles: extend Identity type (4 optional fields) + src/ui/LiveProfiles.tsx (LiveProfiles + LiveProfilesShortcut exports with both/single/neither variants) + tests + mount in Certs.tsx and Contact.tsx (CTC-03)
- [ ] 04-04-PLAN.md — SEO surfaces: pick canonical GH-Pages host + replace public/sitemap.xml (six URLs no changefreq/priority) + reconcile index.html (canonical/og:url/og:image/twitter:image/JSON-LD url+image/title/meta description per UI-SPEC) + verify JSON-LD email omission + update robots.txt Sitemap host (OPS-05 + CTC-03 host alignment)
- [ ] 04-05-PLAN.md — ContactForm + LiveProfilesShortcut mounted in 3D-shell center monitor (CenterMonitorContent.tsx extension; depends on 04-01/04-02/04-03; CTC-01 + CTC-03 single-source-of-truth)
...
| 4. Real Asset, Postprocessing, Polish, Pre-Launch QA | 0/8 | Planned | - |
